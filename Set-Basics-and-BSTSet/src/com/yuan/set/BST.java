...
public class BST<E extends Comparable<E>> {
    ...
    /**（I think）
     * 返回不大于e的上一个元素（e不在二分搜索树中）
     * 换句话说就是二分搜索树中小于e的最大元素
     * @return
     */
    public E floor(E e){
        /*
        先通过中序遍历的形式将二分搜索树中的所有元素存储到动态数组中，然后按从大到小的顺序遍历，
        找到第一个小于e的元素，即是所找元素；
         */
        ArrayList<E> arrayList = new ArrayList<>();
        arrayList = inOrderToArray(arrayList,root);
        System.out.println("floor arrayList: "+arrayList);
        E res = null;

        if (e.compareTo(arrayList.get(0))<0){
            res = null;
        } else{
            /*
            arrayList中是从小到大排序的，所以这里进行逆序遍历，
            从arrayList的末尾元素也就是最大元素往前遍历，直到找到第一个比e小的元素为止；
            */
            for (int i=arrayList.size()-1;i>=0;i--){
                if (e.compareTo(arrayList.get(i)) > 0){
                    res = arrayList.get(i);
                    break;
                }
            }
    ...
    /**（I think）
     * 返回不小于e的下一个元素 （e不在二分搜索树中）
     * 换句话说就是二分搜索树中大于e的最小元素
     * @param e
     * @return
     */
    public E ceil(E e){
        ...
        if (e.compareTo(arrayList.get(arrayList.size()-1))>0){
            return null;
        }else{
            /*
            arrayList中是从小到大排序的，所以这里进行顺序遍历，
            从arrayList的首元素也就是最小元素往后遍历，直到找到第一个比e大的元素为止；
            */
    ...
    public int rank(E e){
        ...
        int res = -1;
        if (e.compareTo(arrayList.get(0))<0 || e.compareTo(arrayList.get(length-1))>0 || length == 0){
            res = -1;
        }else{
    ...
    /**
     * 将二分搜索树按中序遍历存储到动态数组中（即按从大到小的顺序存储）
     * @param arrayList
     * @param node
     * @return
     */
    private ArrayList<E> inOrderToArray(ArrayList<E> arrayList,Node node){
       if (node == null)
           return null;
       inOrderToArray(arrayList,node.left);  // 先访问左子树
       arrayList.add(node.e);    // 再访问根节点
       inOrderToArray(arrayList,node.right);  //最后访问右子树
